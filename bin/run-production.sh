...
export HEALTHCHECK_URI="/authorizer/healthcheck"
...
curl  --url "http://${HTTP_PORT:-unknown}${HEALTHCHECK_URI}" --fail -i --max-time 5 && \
  echo "" && echo "Authorizer and gateway is up (http: ok)" || exit 1
...
curl  --url "https://${HTTPS_PORT:-unknown}${HEALTHCHECK_URI}" --fail -i --max-time 5 --insecure && \
  echo "" && echo "Authorizer and gateway is up (https: ok)" || exit 1

echo "Healthcheck authorizer via gateway with Public URL: $PUBLIC_URL"
curl  --url "${PUBLIC_URL}${HEALTHCHECK_URI}" --fail -i --max-time 5 && \
  echo "" && echo "Authorizer and gateway is up (public: ok)" || exit 1
